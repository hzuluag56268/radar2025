...
#from aircraft import * # Comentado si AircraftModel está en models.py
from views import AircraftSprite, AircraftLabelView # Asegúrate que AircraftSprite también se importe si es necesario
from models import AircraftModel
from ui import ui # Renombrado a self.ui_manager en __init__
...
class Game:
    def __init__(self):
        print('ejercicio 0') # Considera cambiar esto o hacerlo dinámico con exercise_num_str
        ...
        self.elapsed_time = 0.0 # Inicializar como float
        self.ui_manager = ui() # Usar self.ui_manager consistentemente
        self.level_str = ""
        self.exercise_num_str = "1" # Asegúrate que este sea el valor deseado o se obtenga antes de cargar datos
        
        self.routes_config = ROUTES 
        # Cargar datos del ejercicio DESPUÉS de que exercise_num_str esté definido si la carga depende de él
        # o si la estructura de aircraft_creation_data es un dict de dicts por ejercicio.
    ...
    def load_exercise_data(self, file_path):
        try:
            with open(file_path, 'r') as f:
                data = json.load(f)
            # Asegurar que todas las entradas de datos de aeronaves tengan las banderas necesarias
            for ex_num_key in data: # Iterar sobre las claves de ejercicio (ej: "0", "1")
                if isinstance(data[ex_num_key], list): # Asegurarse que es una lista de aeronaves
                    for ac_data in data[ex_num_key]:
                        ac_data.setdefault('_is_authorized_early', False)
                        ac_data.setdefault('_processed_and_killed', False) # Usar esta bandera consistentemente
            return data
                ...
                if acft_data_entry['label'] == aircraft_label_to_launch:
                    if not acft_data_entry.get('_processed_and_killed', False) and \
                       not acft_data_entry.get('_is_authorized_early', False):
                        ...
                        print(f"Game: {aircraft_label_to_launch} ya fue autorizado.")
                    else:
                        print(f"Game: {aircraft_label_to_launch} no pudo ser autorizado (ya procesado o tiempo pasado).")
                    break
        else:
    ...
    def run(self):
        ...
        while self.running:
            ...
            mouse_buttons_pressed = pygame.mouse.get_pressed() # Obtener estado de botones del mouse
            ...
            for event in pygame.event.get():
                ...
                # 1. Manejo de Arrastre de Etiquetas
                ...
                # 2. Manejo de Input del Panel Lateral
                panel_consumed_click = False
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                     if mouse_pos_tuple[0] >= SCREEN_WIDTH: # Optimización: solo chequear si el clic está en el área del panel
                        panel_consumed_click = self.side_panel.handle_panel_input(event, mouse_pos_tuple)
                
                if panel_consumed_click:
                    continue

                # 3. Otros Eventos de UI (Menú, Nivel)
                        ...
                        if self.ui_manager.show_menu:
                            ...
                            if action:
                                ...
                                if action == "close_menu": self.selected_aircraft_model = None
                                    ...
                                    elif action == "Finish Holding Pattern": self.selected_aircraft_model.set_finish_holding(True) # Corregido de self.selected_aircraft
                                    elif action in ["Stop descent at", "Stop climb at", "Continue descent to", "continue climb to"]:
                                        self.ui_manager.display_level_input(mouse_pos_tuple) # Usar mouse_pos_tuple
                                    elif action == "disregard": self.selected_aircraft_model = None # Corregido de self.selected_aircraft
                                    if action not in ["Stop descent at", "Stop climb at", "Continue descent to", "continue climb to"]:
                                        self.selected_aircraft_model = None
            if not self.running: break
            
            # --- Lógica de Actualización ---
            # Pasar el diccionario completo self.aircraft_creation_data
            self.side_panel.update(self.aircraft_models, self.aircraft_creation_data, self.elapsed_time) # <<<--- CORRECCIÓN AQUÍ

            # Lógica de Creación de Aeronaves
            if self.exercise_num_str in self.aircraft_creation_data:
                entries_to_create_this_frame = [] # Para evitar modificar la lista mientras se itera
                ...
                for acft_data in entries_to_create_this_frame:
                    if acft_data.get('_processed_and_killed', False): continue # Doble chequeo
                    ...
                    if route_name in self.routes_config:
                        ...
                        acft_data['_processed_and_killed'] = True # Marcar como procesada
                        # Ya no se remueve de la lista self.aircraft_creation_data[self.exercise_num_str]
                        # para que SidePanel pueda seguir refiriéndose a ella si es necesario (aunque no debería si está killed)
                        # Opcional: Si prefieres remover, hazlo después de este bucle sobre una lista de 'acft_data' a remover.
                    else:
...
if __name__ == '__main__':
    # El input para el número de ejercicio debería manejarse antes de crear Game()
    # o dentro de Game.__init__() si prefieres.
    # screen_for_input = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT)) # Temporal para input
    # font_for_input = pygame.font.Font(None, 36)
    # exercise_num_input = get_exercise_input(screen_for_input, font_for_input) # Asumiendo que esta función existe y funciona
    # pygame.display.quit() # Cerrar la ventana temporal de input

    radar_game = Game()
    # if exercise_num_input is not None:
    #    radar_game.exercise_num_str = str(exercise_num_input)
    # else:
